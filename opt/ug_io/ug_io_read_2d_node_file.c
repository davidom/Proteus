 ...
 * $Id: ug_io_read_2d_node_file.c,v 1.10 2013/03/16 17:09:09 marcum Exp $
 ...
 */
  ...
  if (Read_Task_Flag == 1)
  {
    ...
    {
      Read_Task_Flag = 0;
      strcpy (Error_Message, "unable to uncompress 2D node data file");
      return (637);
    }
  ...
  File_Type_Flag = (strcmp (File_Type_Suffix_,  ".node2d") == 0) ? 1:
                   (strcmp (File_Type_Suffix_, ".snode2d") == 0) ? 2:
                   (strcmp (File_Type_Suffix_, ".vnode2d") == 0) ? 3: 0;
  ...
  if (File_Type_Flag > 0)
  {
    ...
    {
      Read_Task_Flag = 0;
      strcpy (Error_Message, "unable to open 2D node data file");
      return (637);
    }

    Error_Flag = ug_io_read_2d_node (Node_Data_File, Error_Message, 
  ...
  {
    Read_Task_Flag = 0;
    strcpy (Error_Message, "unknown input 2D node data file type");
    return (637);
  }
