 ...
 * $Id: ug_get_double_data.c,v 1.14 2013/03/16 19:11:06 marcum Exp $
 */
  ...
  if (ug_file_status_monitor_flag())
  {
    ...
    sprintf (Text, "ug_get_double_data       : reading data from TMP file %s",
             Name_Text);
    ug_message (Text);
  }
  ...
  if (mread >= 2 && *n > 0)
  {
    iread = iread + ug_fread (data, sizeof (double), dim*(*n), Data_File);
  ...
  if (ug_file_status_monitor_flag())
  {
    sprintf (Text, "ug_get_double_data       : read %i items of %i-dim data", (int) *n, (int) dim);
    ug_message (Text);
  }
